...
namespace BibliotecaAPITest.Utilidades
{
    public class BasePruebas
    {
        ...
        {
            var urlRegistro = "api/v1/usuarios/registros";
            ...
            if (claims.Any())
            {
                ...
                var usuario = await context.Users.Where(x => x.Email == token).FirstAsync();
                Assert.IsNotNull(usuario);

                var usarClaims = claims.Select(x => new IdentityUserClaim<string>
                {
                    UserId = usuario.Id,
                    ClaimType = x.Type,
                    ClaimValue = x.Value
                ...
                context.UserClaims.AddRange(usarClaims);
        ...
        private async Task<string> ObtenerToken(string email, string url, WebApplicationFactory<Program> factory)
        {
            ...
            Assert.IsNotNull(respuestaAutenticacion);
